  ...
  return (
    <div style={{ marginBottom: token.marginLG }}>
      ...
      <Card title="项目介绍" style={{ marginBottom: token.marginLG }}>
        <div>bjsh98.db（大体协数据库）是一个为北京市大学生体育协会（简称“大体协”）成绩查询页开发的数据检索平台，借助于信息化技术，尤其是软件技术和互联网技术，支持赛会组织方、运动员、观众等群体高效、安全地上传、修正、查询相关赛事成绩数据，优化落后的查询模式。</div>
        <br/>
        <div>目前前后端都是 render 的 free plan，每个月限 500M 的流量，并且一段时间没有访问后会自动进入休眠。目前看来前端发给后端的请求并不能将后端唤醒，因此如果发现检索返回都是 Too many requests，可以手动在浏览器里输入 https://bjsh98db-backend.onrender.com，等待 404 页面出现就可以正常使用前端了。</div>
      </Card>
